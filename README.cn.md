...
zhell 会追踪你的使用积分。提示符会显示历史总分：`zhell [42]>`。随时输入 `分数` 查看今日、本周、历史总分及计分规则。
...
- 输入 `帮` 或 `分数` 不计分也不扣分。
- 分数不会低于 0。
- 今日、本周、历史总分分别独立记录于 `~/.zhell_score.json`。
...
| 分数     | fēnshù       | `score`         | 显示你的分数及计分规则                    |
| 提权     | tí quán      | `sudo`          | 以超级用户权限运行 zhell 命令         |
| 设       | shè          | `export`        | 设置环境变量                      |
